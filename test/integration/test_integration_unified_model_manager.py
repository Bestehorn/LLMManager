...
class TestUnifiedModelManagerBasicFunctionality:
    ...
    def test_unified_model_manager_load_cached_data(self, tmp_path) -> None:
        ...
        manager = UnifiedModelManager(json_output_path=json_output_path, force_download=False)
            ...
            if catalog.model_count > 0:
                ...
                for model_name in claude_models:
                    access_info = manager.get_model_access_info(
                        model_name=model_name, region=test_region
                    )
...
class TestUnifiedModelManagerConfiguration:
    ...
    def test_unified_model_manager_fuzzy_matching(self, tmp_path) -> None:
        ...
        manager_with_fuzzy = UnifiedModelManager(
            json_output_path=json_output_path, force_download=False, enable_fuzzy_matching=True
        )
        ...
        manager_without_fuzzy = UnifiedModelManager(
            json_output_path=json_output_path, force_download=False, enable_fuzzy_matching=False
        )
    ...
    def test_unified_model_manager_repr(self, tmp_path) -> None:
        ...
        manager = UnifiedModelManager(
            json_output_path=json_output_path, force_download=True, enable_fuzzy_matching=True
        )
