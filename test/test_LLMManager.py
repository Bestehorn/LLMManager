...
class TestLLMManager:
    ...
    def basic_llm_manager(self, mock_unified_model_manager):
        ...
        with patch('LLMManager.UnifiedModelManager', return_value=mock_unified_model_manager):
            return LLMManager(
    ...
    def test_validate_converse_request_missing_content(self, basic_llm_manager):
        ...
        with pytest.raises(RequestValidationError, match="Message 0 missing required 'content' field"):
            basic_llm_manager._validate_converse_request([message])
    ...
    def test_validate_content_blocks_image_limit_exceeded(self, basic_llm_manager):
        ...
        errors = []
        content_blocks = [{"image": {"format": "png"}} for _ in range(ContentLimits.MAX_IMAGES_PER_REQUEST + 1)]
    ...
    def test_validate_content_blocks_document_limit_exceeded(self, basic_llm_manager):
        ...
        errors = []
        content_blocks = [{"document": {"name": "test.pdf"}} for _ in range(ContentLimits.MAX_DOCUMENTS_PER_REQUEST + 1)]
    ...
    def test_validate_content_blocks_video_limit_exceeded(self, basic_llm_manager):
        ...
        errors = []
        content_blocks = [{"video": {"format": "mp4"}} for _ in range(ContentLimits.MAX_VIDEOS_PER_REQUEST + 1)]
    ...
    def test_build_converse_request_merges_default_inference_config(self, mock_unified_model_manager):
    ...
    def test_refresh_model_data_failure(self, basic_llm_manager):
        ...
        basic_llm_manager._unified_model_manager.refresh_unified_data.side_effect = Exception("Network error")
...
class TestLLMManagerIntegration:
    ...
    def test_converse_success_flow(self, mock_datetime, mock_components):
        ...
        with patch('LLMManager.UnifiedModelManager', return_value=mock_components['unified_model_manager']), \
             patch('LLMManager.AuthManager', return_value=mock_components['auth_manager']), \
             patch('LLMManager.RetryManager', return_value=mock_components['retry_manager']):
...
class TestLLMManagerUncoveredCases:
    ...
    def test_init_unified_model_manager_load_cached_fails(self):
        ...
        with patch('LLMManager.UnifiedModelManager', return_value=mock_unified_manager):
            manager = LLMManager(
                models=["Claude 3 Haiku"],
                regions=["us-east-1"]
            )
    ...
    def test_execute_converse_success(self, basic_llm_manager):
        ...
        with patch.object(basic_llm_manager._auth_manager, 'get_bedrock_client', return_value=mock_client):
    ...
    def test_execute_converse_no_region_available(self, basic_llm_manager):
        ...
        with patch.object(basic_llm_manager._auth_manager, 'get_bedrock_client', side_effect=Exception("Auth failed")):
            with pytest.raises(AuthenticationError, match="Could not authenticate to any specified region"):
    ...
    def test_get_model_access_info_exception(self, basic_llm_manager):
        ...
        basic_llm_manager._unified_model_manager.get_model_access_info.side_effect = Exception("Error")
    ...
    def test_validate_configuration_auth_error(self, basic_llm_manager):
        ...
        with patch.object(basic_llm_manager._auth_manager, 'get_auth_info', side_effect=Exception("Auth error")):
            result = basic_llm_manager.validate_configuration()
