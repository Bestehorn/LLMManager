...
class TestModelManager:
    ...
    def test_refresh_model_data_success(self, model_manager, mock_downloader, mock_parser, mock_serializer):
    ...
    def test_refresh_model_data_network_error(self, model_manager, mock_downloader):
        ...
        with pytest.raises(ModelManagerError, match="Failed to refresh model data: Connection failed"):
    ...
    def test_refresh_model_data_parsing_error(self, model_manager, mock_parser):
        ...
        with pytest.raises(ModelManagerError, match="Failed to refresh model data: Invalid HTML structure"):
            model_manager.refresh_model_data()
    ...
    def test_refresh_model_data_file_system_error(self, model_manager, mock_serializer):
        ...
        with pytest.raises(ModelManagerError, match="Failed to refresh model data: Permission denied"):
            model_manager.refresh_model_data()
    
    def test_refresh_model_data_skip_download_if_recent(self, model_manager, mock_downloader, tmp_path):
    ...
    def test_download_documentation(self, model_manager, mock_downloader):
        ...
        mock_downloader.download.assert_called_once_with(
            url=model_manager.documentation_url,
            output_path=model_manager.html_output_path
        )
    ...
    def test_save_catalog_to_json(self, model_manager, mock_serializer):
        ...
        mock_serializer.serialize_to_file.assert_called_once_with(
            catalog=catalog,
            output_path=model_manager.json_output_path
        )
