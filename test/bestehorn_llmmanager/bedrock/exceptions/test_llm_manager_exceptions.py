...
class TestAuthenticationError:
    ...
    def test_initialization_with_both_parameters(self):
        ...
        error = AuthenticationError("Auth failed", auth_type="iam", region="eu-west-1")
...
class TestModelAccessError:
    ...
    def test_initialization_with_all_parameters(self):
        ...
        error = ModelAccessError(
            "Access failed", model_id="claude-3-sonnet", region="us-west-2", access_method="direct"
        )
...
class TestRetryExhaustedError:
    ...
    def test_initialization_with_models_and_regions(self):
        ...
        error = RetryExhaustedError("Retries failed", models_tried=models, regions_tried=regions)

        assert error.models_tried == models
        assert error.regions_tried == regions
        assert error.details == {"models_tried": models, "regions_tried": regions}
    ...
    def test_initialization_with_none_lists(self):
        ...
        error = RetryExhaustedError(
            "Failed", last_errors=None, models_tried=None, regions_tried=None
        )
...
class TestRequestValidationError:
    ...
    def test_initialization_with_both_parameters(self):
        ...
        error = RequestValidationError(
            "Validation failed", validation_errors=validation_errors, invalid_fields=invalid_fields
        )
    ...
    def test_initialization_with_none_lists(self):
        """Test initialization with None lists."""
        error = RequestValidationError("Failed", validation_errors=None, invalid_fields=None)
...
class TestStreamingError:
    ...
    def test_initialization_with_both_parameters(self):
        ...
        error = StreamingError("Stream failed", stream_position=position, partial_content=content)

        assert error.stream_position == position
        assert error.partial_content == content
        assert error.details == {"stream_position": position, "partial_content": content}
...
class TestContentError:
    ...
    def test_initialization_with_sizes(self):
        ...
        error = ContentError("Content too large", content_size=1048576, max_allowed_size=524288)

        assert error.content_size == 1048576
        assert error.max_allowed_size == 524288
        assert error.details == {"content_size": 1048576, "max_allowed_size": 524288}
    ...
    def test_initialization_with_zero_sizes(self):
        ...
        error = ContentError("Empty content", content_size=0, max_allowed_size=0)

        assert error.content_size == 0
        assert error.max_allowed_size == 0
        assert error.details == {"content_size": 0, "max_allowed_size": 0}
