...
class TestParallelLLMManagerAdvanced:
    """Test advanced ParallelLLMManager functionality."""
    ...
    def test_get_execution_stats(self) -> None:
        ...
        with patch("bestehorn_llmmanager.parallel_llm_manager.LLMManager"):
            ...
            with patch.object(
                parallel_manager._parallel_executor, "get_execution_stats", return_value=mock_stats
            ):
                stats = parallel_manager.get_execution_stats()

                assert stats["total_requests"] == 10
                assert stats["successful_requests"] == 8
                assert stats["success_rate"] == 0.8

    def test_get_region_load_distribution(self) -> None:
        ...
        with patch("bestehorn_llmmanager.parallel_llm_manager.LLMManager"):
            ...
            mock_distribution = {"us-east-1": 5, "us-west-2": 3}

            with patch.object(
                parallel_manager._region_distributor,
                "get_region_load_distribution",
                return_value=mock_distribution,
            ):
                distribution = parallel_manager.get_region_load_distribution()

                assert distribution["us-east-1"] == 5
                assert distribution["us-west-2"] == 3

    def test_set_load_balancing_strategy(self) -> None:
        """Test setting load balancing strategy."""
        with patch("bestehorn_llmmanager.parallel_llm_manager.LLMManager"):
            parallel_manager = ParallelLLMManager(models=["claude-3-haiku"], regions=["us-east-1"])

            with patch.object(parallel_manager._region_distributor, "set_load_balancing_strategy"):
                parallel_manager.set_load_balancing_strategy(LoadBalancingStrategy.LEAST_LOADED)

                parallel_manager._region_distributor.set_load_balancing_strategy.assert_called_once_with(
                    LoadBalancingStrategy.LEAST_LOADED
                )

    def test_reset_load_tracking(self) -> None:
        """Test resetting load tracking."""
        with patch("bestehorn_llmmanager.parallel_llm_manager.LLMManager"):
            parallel_manager = ParallelLLMManager(models=["claude-3-haiku"], regions=["us-east-1"])

            with patch.object(parallel_manager._region_distributor, "reset_load_tracking"):
                parallel_manager.reset_load_tracking()

                parallel_manager._region_distributor.reset_load_tracking.assert_called_once()
    ...
    def test_get_available_models(self) -> None:
        ...
        with patch(
            ...
            mock_llm_manager = Mock()
            mock_llm_manager.get_available_models.return_value = models
            mock_llm_manager_class.return_value = mock_llm_manager

            parallel_manager = ParallelLLMManager(models=models, regions=["us-east-1"])

            result = parallel_manager.get_available_models()

            assert result == models

    def test_get_available_regions(self) -> None:
        ...
        with patch(
            ...
            mock_llm_manager = Mock()
            mock_llm_manager.get_available_regions.return_value = regions
            mock_llm_manager_class.return_value = mock_llm_manager

            parallel_manager = ParallelLLMManager(models=["claude-3-haiku"], regions=regions)

            result = parallel_manager.get_available_regions()

            assert result == regions

    def test_converse_parallel_with_custom_processing_config(self) -> None:
        """Test parallel processing with custom configuration."""
        models = ["claude-3-haiku"]
        regions = ["us-east-1", "us-west-2"]
        ...
        custom_config = ParallelProcessingConfig(
            max_concurrent_requests=10,
            request_timeout_seconds=120,
            failure_handling_strategy=FailureHandlingStrategy.CONTINUE_ON_ERROR,
        )
            ...
            with patch.object(parallel_manager._request_validator, "validate_batch_requests"):
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        mock_distribute.return_value = [
                            Mock(request_id="req1", assigned_regions=["us-east-1"]),
                        ]
                        mock_execute.return_value = mock_responses

                        result = parallel_manager.converse_parallel(
                            requests, target_regions_per_request=2
                        )
