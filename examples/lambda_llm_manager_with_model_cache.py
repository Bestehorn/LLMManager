...
catalog = BedrockModelCatalog(
    cache_mode=CacheMode.MEMORY, cache_max_age_hours=24.0, fallback_to_bundled=True
)
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Catalog source: {metadata.source.value} " f"(memory-only, no file I/O)")
        ...
        if preferred_model:
            ...
            temp_manager = LLMManager(
                models=[preferred_model], regions=["us-east-1", "us-west-2"], log_level=logging.INFO
            )
        ...
        if preferred_model:
            is_available = catalog.is_model_available(
                model_name=preferred_model, region="us-east-1"
            )
...
if __name__ == "__main__":
    ...
    test_event = {"prompt": "What is the capital of France?"}
    result = lambda_handler(event=test_event, context=None)
    print(json.dumps(result, indent=2))

    # Test 2: Model availability check
    print("\nTest 2: Model availability check")
    availability_event = {"model": "Claude 3 Haiku", "region": "us-east-1"}
