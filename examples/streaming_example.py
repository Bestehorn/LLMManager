...
def basic_streaming_example():
    ...
    manager = LLMManager(models=models, regions=regions, log_level=logging.INFO)
    ...
    messages = [
        {
            ...
            "content": [
                {
                    "text": "Write a short story about a robot learning to paint. Make it about 3 paragraphs."
                }
            ],
        }
    ]
    ...
    try:
        ...
        print(
            f"   Duration: {streaming_response.total_duration_ms:.1f}ms"
            if streaming_response.total_duration_ms
            else "   Duration: N/A"
        )
        print(
            f"   API Latency: {streaming_response.api_latency_ms:.1f}ms"
            if streaming_response.api_latency_ms
            else "   API Latency: N/A"
        )
...
def streaming_with_iterator_example():
    ...
    # Initialize manager
    # The new BedrockModelCatalog handles data fetching automatically
    manager = LLMManager(models=models, regions=regions, log_level=logging.WARNING)
    ...
    try:
        ...
        if metrics:
            print("⏱️  Enhanced Timing Metrics:")
            ...
            if "token_generation_duration_ms" in metrics:
                print(
                    f"   Token generation duration: {metrics['token_generation_duration_ms']:.1f}ms"
                )
...
def streaming_error_recovery_example():
    ...
    retry_config = RetryConfig(
        max_retries=4, retry_delay=0.5, max_retry_delay=8.0, backoff_multiplier=1.8
    )
    ...
    print(
        f"📊 Configuration: {len(models)} models × {len(regions)} regions = {len(models) * len(regions)} possible attempts"
    )
    ...
    try:
        ...
        if streaming_response.success:
            ...
            print(f"📦 Content parts: {len(streaming_response.content_parts)}")
            print(
                f"⏱️  Duration: {streaming_response.total_duration_ms:.1f}ms"
                if streaming_response.total_duration_ms
                else "⏱️  Duration: N/A"
            )
            print(f"🔚 Stop reason: {streaming_response.stop_reason}")
            ...
            if recovery_info.get("recovery_enabled", False):
                ...
                if mid_stream_exceptions:
                    ...
                    for i, exc in enumerate(mid_stream_exceptions, 1):
                        ...
                        print(
                            f"     {i}. {exc['error_type']} at position {exc['position']} ({exc['model']}, {exc['region']}) - {status}"
                        )
                        if len(exc["error_message"]) < 100:
                            print(f"        Error: {exc['error_message']}")
                else:
                    print("   No mid-stream exceptions occurred")

                if (
                    recovery_info.get("final_model") != models[0]
                    or recovery_info.get("final_region") != regions[0]
                ):
                    print(
                        f"   🔄 Stream switched to: {recovery_info.get('final_model')} in {recovery_info.get('final_region')}"
                    )
            ...
            if metrics:
                ...
                if "token_generation_duration_ms" in metrics:
                    print(
                        f"   Token generation time: {metrics['token_generation_duration_ms']:.1f}ms"
                    )
            ...
            if recovery_info.get("recovery_enabled", False):
                print("   Recovery attempts made:")
                print(f"     Total exceptions: {recovery_info.get('total_exceptions', 0)}")
                print(f"     Recovered exceptions: {recovery_info.get('recovered_exceptions', 0)}")
                print(f"     Target switches: {recovery_info.get('target_switches', 0)}")

                if mid_stream_exceptions:
                    print("   Mid-stream exceptions:")
                    for i, exc in enumerate(mid_stream_exceptions, 1):
                        status = "recovered" if exc["recovered"] else "failed"
...
def compare_streaming_vs_regular():
    ...
    manager = LLMManager(
        models=models, regions=regions, log_level=logging.WARNING  # Reduce noise for comparison
    )
    ...
    try:
        ...
        print(f"   ✓ Success: {streaming_response.success}")
        duration_str = (
            f"{streaming_response.total_duration_ms:.1f}ms"
            if streaming_response.total_duration_ms is not None
            else "N/A"
        )
