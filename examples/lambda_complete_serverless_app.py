...
def handle_converse(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Catalog source: {metadata.source.value} " f"(no caching, fresh data)")
...
def handle_list_models(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        models = catalog.list_models(
            region=region, provider=provider, streaming_only=streaming_only
        )
...
def handle_check_availability(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        model_info = None
        if is_available:
            info = catalog.get_model_info(model_name=model_name, region=region)
            if info:
                model_info = {
...
if __name__ == "__main__":
    ...
    print("-" * 60)
    list_event = {"operation": "list_models", "region": "us-east-1", "streaming_only": False}
