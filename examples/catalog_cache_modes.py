...
def measure_initialization_time(cache_mode: CacheMode, **kwargs) -> Dict[str, Any]:
    """
...
def example_1_file_cache_mode():
    ...
    result2 = measure_initialization_time(
        cache_mode=CacheMode.FILE, cache_directory=Path("./demo_cache")
    )
    ...
    catalog = BedrockModelCatalog(cache_mode=CacheMode.FILE, cache_directory=Path("./demo_cache"))
    metadata = catalog.get_catalog_metadata()
...
def example_5_performance_comparison():
    ...
    file_result = measure_initialization_time(
        cache_mode=CacheMode.FILE, cache_directory=Path("./perf_test_cache")
    )
...
def example_6_cache_configuration():
    ...
    BedrockModelCatalog(cache_mode=CacheMode.FILE, cache_directory=Path("./my_app_cache"))
    print("  Cache directory: ./my_app_cache")

    print("\n6b. Custom cache age:")
    BedrockModelCatalog(cache_mode=CacheMode.FILE, cache_max_age_hours=6.0)  # Refresh every 6 hours
    print("  Max cache age: 6 hours")

    print("\n6c. Disable bundled fallback:")
    try:
        BedrockModelCatalog(
            cache_mode=CacheMode.NONE, fallback_to_bundled=False  # Fail if API unavailable
        )
        print("  Bundled fallback: disabled")
    except Exception as e:
        print(f"  Failed (expected if no API access): {type(e).__name__}")

    print("\n6d. Custom API timeout:")
    BedrockModelCatalog(cache_mode=CacheMode.FILE, timeout=10)  # 10 second timeout
...
def example_7_cache_invalidation():
    ...
    catalog1 = BedrockModelCatalog(
        cache_mode=CacheMode.FILE, cache_max_age_hours=24.0  # Default: 24 hours
    )
    ...
    catalog2 = BedrockModelCatalog(
        cache_mode=CacheMode.FILE, force_refresh=True  # Always fetch fresh
    )
