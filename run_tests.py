...
def _is_aws_integration_explicitly_requested(args) -> bool:
    ...
    """
    return any(
        [args.aws_integration, args.aws_fast, args.aws_low_cost, args.aws_profile is not None]
    )
...
Examples:
  ...
  python run_tests.py test/unit/test_a.py test/unit/test_b.py              # Run multiple test files
    ...
    parser.add_argument(
        "--unit", action="store_true", help="Run only unit tests (marked with @pytest.mark.unit)"
    )
    ...
    parser.add_argument(
        "--aws-fast", action="store_true", help="Run only fast AWS integration tests (< 30 seconds)"
    )
    ...
    parser.add_argument(
        "--all", action="store_true", help="Run both mocked unit tests and AWS integration tests"
    )

    parser.add_argument(
        "--fast", action="store_true", help="Skip slow tests (marked with @pytest.mark.slow)"
    )

    parser.add_argument(
        "--no-coverage", action="store_true", help="Run tests without coverage reporting"
    )

    parser.add_argument("--html", action="store_true", help="Generate HTML test report")

    parser.add_argument(
        "--parallel", action="store_true", help="Run tests in parallel using pytest-xdist"
    )
    ...
    parser.add_argument(
        "--install-deps", action="store_true", help="Install test dependencies before running tests"
    )
    ...
    if not args.no_coverage:
        ...
        if args.html:
            pytest_cmd.extend(
                ["--cov-report=html:htmlcov", "--html=test_results.html", "--self-contained-html"]
            )
    elif args.html:
        pytest_cmd.extend(["--html=test_results.html", "--self-contained-html"])
