...
from ..models.cris_structures import CRISModelInfo, CRISCatalog
...
class ModelCRISCorrelator:
            ...
            for cris_model_name, cris_model_info in cris_catalog.cris_models.items():
                ...
                try:
                    ...
                    unified_model = self._create_cris_only_unified_model(
                        cris_model_info=cris_model_info,
                    ...
                    raise ModelCRISCorrelationError(f"CRIS model correlation failed for '{cris_model_name}': {error_context}") from e
    ...
    def _create_model_name_mapping(
        self,
    ...
    def _find_matching_cris_model(
        ...
        if self._fuzzy_matching_enabled:
                ...
                if normalized_cris == normalized_target:
                    ...
                    self._logger.warning(
                        UnifiedLogMessages.FUZZY_MATCH_APPLIED.format(
                            regular_model=model_name,
                            cris_model=cris_name
                        )
                    )
            ...
            self._logger.debug(
                UnifiedLogMessages.FUZZY_MATCHING_DISABLED.format(
                    model_name=model_name
                )
            )
        ...
        try:
            ...
            for region in cris_model_info.get_source_regions():
                try:
                    ...
                    inference_profiles = cris_model_info.get_profiles_for_source_region(source_region=region)
                    primary_profile = inference_profiles[0] if inference_profiles else cris_model_info.inference_profile_id
        ...
        for region in model_info.regions_supported:
            try:
                ...
                if region.endswith(RegionMarkers.CRIS_ONLY_MARKER):
                    ...
                    cris_available = (
                        cris_model_info and 
                        cris_model_info.can_route_from_source(source_region=region)
                    )
    ...
    def _get_unmatched_regular_models(
        ...
        return [
            model_name for model_name in model_catalog.models.keys()
    ...
    def _log_correlation_results(
        self,
        unmatched_regular_models: List[str],
        unmatched_cris_models: List[str]
    ) -> None:
        """Log the results of the correlation process."""
        total_matched = self._correlation_stats["matched_models"] + self._correlation_stats["fuzzy_matched_models"]
        
        self._logger.info(
            UnifiedLogMessages.CORRELATION_COMPLETED.format(
        ...
        if unmatched_regular_models:
            self._logger.warning(
                UnifiedLogMessages.UNMATCHED_MODELS.format(
                    count=len(unmatched_regular_models),
                    models=", ".join(unmatched_regular_models)
                )
            )
        ...
        if unmatched_cris_models:
            self._logger.warning(
                UnifiedLogMessages.UNMATCHED_CRIS.format(
                    count=len(unmatched_cris_models),
                    models=", ".join(unmatched_cris_models)
                )
            )
    ...
    def set_fuzzy_matching_enabled(self, enabled: bool) -> None:
        ...
        self._logger.info(
            f"Fuzzy matching {'enabled' if enabled else 'disabled'}"
        )
