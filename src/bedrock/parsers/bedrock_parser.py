...
from ..models.constants import HTMLTableColumns, BooleanValues, LogMessages
...
class BedrockHTMLParser(BaseDocumentationParser):
    ...
    def parse(self, file_path: Path) -> Dict[str, BedrockModelInfo]:
        ...
        try:
            ...
            self._logger.info(
                LogMessages.PARSING_COMPLETED.format(model_count=len(models))
            )
    ...
    def _extract_model_from_row(self, row: Tag) -> tuple[Optional[str], Optional[BedrockModelInfo]]:
        ...
        try:
            ...
            provider = self._extract_text_from_cell(
                cells=cells, column=HTMLTableColumns.PROVIDER
            )
            
            model_id = self._extract_text_from_cell(
                cells=cells, column=HTMLTableColumns.MODEL_ID
            )
