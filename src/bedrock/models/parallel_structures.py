...
class BedrockConverseRequest:
    ...
    def _generate_request_id(self) -> str:
        ...
        content_hash = hashlib.sha256(content_str.encode()).hexdigest()[:ParallelConfig.REQUEST_ID_HASH_LENGTH]
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'BedrockConverseRequest':
        ...
        return cls(
            ...
            additional_model_request_fields=data.get(ParallelProcessingFields.ADDITIONAL_MODEL_REQUEST_FIELDS),
            additional_model_response_field_paths=data.get(ParallelProcessingFields.ADDITIONAL_MODEL_RESPONSE_FIELD_PATHS),
...
class ParallelProcessingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.request_timeout_seconds <= 0:
            raise ValueError(f"request_timeout_seconds must be positive, got: {self.request_timeout_seconds}")
        if not (0.0 <= self.failure_threshold <= 1.0):
            raise ValueError(f"failure_threshold must be between 0.0 and 1.0, got: {self.failure_threshold}")
