...
class LLMManagerLogMessages:
    ...
    MANAGER_INITIALIZED: Final[str] = "LLMManager initialized with {model_count} models and {region_count} regions"
    AUTH_CONFIGURED: Final[str] = "Authentication configured using {auth_type}"
    UNIFIED_MODEL_MANAGER_LOADED: Final[str] = "UnifiedModelManager loaded with {model_count} models"
    
    # Request processing messages
    REQUEST_STARTED: Final[str] = "Starting converse request with model '{model}' in region '{region}'"
    REQUEST_RETRY: Final[str] = "Retrying request (attempt {attempt}/{max_attempts}) with model '{model}' in region '{region}'"
    REQUEST_FALLBACK: Final[str] = "Falling back from {from_method} to {to_method} for model '{model}' in region '{region}'"
    FEATURE_DISABLED: Final[str] = "Disabled feature '{feature}' for model '{model}' due to incompatibility"
    REQUEST_SUCCEEDED: Final[str] = "Request succeeded with model '{model}' in region '{region}' after {attempts} attempts"
    ...
    ALL_ATTEMPTS_EXHAUSTED: Final[str] = "All retry attempts exhausted. Final attempt with model '{model}' in region '{region}'"
    
    # Performance messages
    REQUEST_TIMING: Final[str] = "Request completed in {duration_ms}ms (API latency: {api_latency_ms}ms)"
...
class LLMManagerErrorMessages:
    ...
    CONTENT_SIZE_EXCEEDED: Final[str] = "Content size exceeds limit: {size} bytes (max: {max_size} bytes)"
    
    # Authentication errors
    CREDENTIALS_NOT_FOUND: Final[str] = "AWS credentials not found. Configure credentials using AWS CLI, environment variables, or IAM roles"
    INVALID_PROFILE: Final[str] = "AWS profile '{profile}' not found"
    PERMISSION_DENIED: Final[str] = "Permission denied for bedrock:InvokeModel operation in region '{region}'"
    
    # Retry errors
    ALL_RETRIES_FAILED: Final[str] = "All retry attempts failed across {model_count} models and {region_count} regions"
