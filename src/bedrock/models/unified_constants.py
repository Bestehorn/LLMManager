...
class UnifiedLogMessages:
    ...
    CORRELATION_COMPLETED: Final[str] = "Model-CRIS correlation completed. Matched {matched_count} models"
    ...
    CRIS_MARKER_DETECTED: Final[str] = "Detected CRIS-only region marker (*) for model {model_name} in region {region}"
    FUZZY_MATCH_APPLIED: Final[str] = "Applied fuzzy matching for model '{regular_model}' -> CRIS model '{cris_model}'. No exact mapping was found, falling back to normalized name matching"
    FUZZY_MATCHING_DISABLED: Final[str] = "Fuzzy matching is disabled. Model '{model_name}' could not be matched to any CRIS model"
    CORRELATION_CONFIG_LOADED: Final[str] = "Model correlation configuration loaded. Fuzzy matching enabled: {fuzzy_enabled}"
