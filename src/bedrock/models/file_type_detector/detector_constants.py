...
class MagicBytesConstants:
    ...
    GIF_SIGNATURES: Final[List[bytes]] = [
        b'GIF87a',
        b'GIF89a'
    ]
    ...
    # Text formats
    HTML_SIGNATURES: Final[List[bytes]] = [
        b'<!DOCTYPE html',
        b'<!doctype html',
        b'<html',
        b'<HTML'
...
class DetectorLogMessages:
    ...
    DETECTION_COMPLETED: Final[str] = "Detection completed for '{filename}': {format} (method: {method}, confidence: {confidence:.2f})"
    
    # Method-specific messages
    EXTENSION_DETECTION: Final[str] = "Extension detection for '{filename}': {extension} -> {format}"
    ...
    # Validation messages
    FORMAT_MISMATCH: Final[str] = "Format mismatch for '{filename}': extension={ext_format}, content={content_format}"
    CONFIDENCE_ADJUSTMENT: Final[str] = "Confidence adjusted for '{filename}': {old_confidence:.2f} -> {new_confidence:.2f} (reason: {reason})"
...
class DetectorErrorMessages:
    ...
    INSUFFICIENT_CONTENT: Final[str] = "Insufficient content for magic byte detection (got {size} bytes, need {required})"
    
    # Detection failures
    NO_SIGNATURE_MATCH: Final[str] = "No magic byte signature matched for content"
    UNSUPPORTED_FORMAT: Final[str] = "Detected format '{format}' is not supported for {content_type}"
    ...
    CONFIDENCE_TOO_LOW: Final[str] = "Detection confidence ({confidence:.2f}) below minimum threshold ({min_threshold:.2f})"
