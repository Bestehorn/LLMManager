...
class FileTypeDetector(BaseDetector):
    ...
    def detect_image_format(
        ...
        if not self._validate_content(content=content):
            return self._create_error_result(
                error_message=DetectorErrorMessages.CONTENT_READ_ERROR.format(error="Invalid content"),
                filename=filename,
    ...
    def _detect_image_by_content(
        ...
        if (header.startswith(MagicBytesConstants.WEBP_SIGNATURE) and 
            len(content) >= 12 and 
            MagicBytesConstants.WEBP_FORMAT_SIGNATURE in content[:12]):
            self._logger.debug(
                DetectorLogMessages.CONTENT_DETECTION.format(
                    filename=safe_filename,
                    format="webp"
    ...
    def _detect_video_by_content(
        ...
        if (header.startswith(MagicBytesConstants.AVI_SIGNATURE) and 
            len(content) >= 12 and 
            MagicBytesConstants.AVI_FORMAT_SIGNATURE in content[:12]):
            return self._create_success_result(
                detected_format="avi",
                confidence=DetectionConstants.HIGH_CONFIDENCE,
                detection_method=DetectionMethodEnum.CONTENT,
    ...
    def _combine_detection_results(
        ...
        if (extension_result and content_result and 
            extension_result.detected_format == content_result.detected_format):
            return self._create_success_result(
