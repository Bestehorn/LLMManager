...
class LLMManager:
    ...
    def _validate_converse_request(self, messages: List[Dict[str, Any]]) -> None:
        ...
        for i, message in enumerate(messages):
            ...
            elif message[ConverseAPIFields.ROLE] not in [ConverseAPIFields.ROLE_USER, ConverseAPIFields.ROLE_ASSISTANT]:
                validation_errors.append(f"Message {i} has invalid role: {message[ConverseAPIFields.ROLE]}")
            ...
            if ConverseAPIFields.CONTENT not in message:
                ...
                self._validate_content_blocks(message[ConverseAPIFields.CONTENT], i, validation_errors)
    ...
    def _validate_content_blocks(self, content_blocks: List[Dict], message_index: int, errors: List[str]) -> None:
        ...
        if image_count > ContentLimits.MAX_IMAGES_PER_REQUEST:
            errors.append(f"Message {message_index} exceeds image limit: {image_count} > {ContentLimits.MAX_IMAGES_PER_REQUEST}")
        
        if document_count > ContentLimits.MAX_DOCUMENTS_PER_REQUEST:
            errors.append(f"Message {message_index} exceeds document limit: {document_count} > {ContentLimits.MAX_DOCUMENTS_PER_REQUEST}")
        
        if video_count > ContentLimits.MAX_VIDEOS_PER_REQUEST:
            errors.append(f"Message {message_index} exceeds video limit: {video_count} > {ContentLimits.MAX_VIDEOS_PER_REQUEST}")
    ...
    def _build_converse_request(
        ...
        request_args: Dict[str, Any] = {
            ConverseAPIFields.MESSAGES: messages
        }
        ...
        if additional_model_request_fields:
            request_args[ConverseAPIFields.ADDITIONAL_MODEL_REQUEST_FIELDS] = additional_model_request_fields
        
        if additional_model_response_field_paths:
            request_args[ConverseAPIFields.ADDITIONAL_MODEL_RESPONSE_FIELD_PATHS] = additional_model_response_field_paths
