...
class MessageBuilderLogMessages:
    ...
    AUTO_DETECTION_SUCCESS: Final[str] = "Auto-detected file type for '{filename}': {detected_type} (method: {method}, confidence: {confidence:.2f})"
    AUTO_DETECTION_FALLBACK: Final[str] = "Using fallback detection for '{filename}': {detected_type} (confidence: {confidence:.2f})"
    DETECTION_MISMATCH: Final[str] = "Format mismatch for '{filename}': extension suggests {ext_type}, content suggests {content_type}, using {final_type}"
    DETECTION_LOW_CONFIDENCE: Final[str] = "Low confidence detection for '{filename}': {detected_type} (confidence: {confidence:.2f})"
    
    # Builder messages
    MESSAGE_BUILD_STARTED: Final[str] = "Building message with role '{role}' and {block_count} content blocks"
    MESSAGE_BUILD_COMPLETED: Final[str] = "Message built successfully with {block_count} content blocks"
    CONTENT_BLOCK_ADDED: Final[str] = "Added {content_type} content block (size: {size} bytes)"
    
    # Validation messages
    CONTENT_SIZE_WARNING: Final[str] = "Content size ({size} bytes) approaching limit ({limit} bytes) for {content_type}"
    CONTENT_BLOCK_LIMIT_WARNING: Final[str] = "Content block count ({count}) approaching limit ({limit})"
...
class MessageBuilderErrorMessages:
    ...
    INVALID_FORMAT: Final[str] = "Invalid {content_type} format: {format}. Supported formats: {supported_formats}"
    
    # Size limit errors
    CONTENT_SIZE_EXCEEDED: Final[str] = "Content size ({size} bytes) exceeds limit ({limit} bytes) for {content_type}"
    CONTENT_BLOCK_LIMIT_EXCEEDED: Final[str] = "Cannot add more content blocks. Maximum allowed: {limit}"
    ...
    DETECTION_CONFIDENCE_LOW: Final[str] = "Detection confidence too low ({confidence:.2f}) for {filename}. Minimum required: {min_confidence:.2f}"
...
class SupportedFormats:
    ...
    DOCUMENT_FORMATS: Final[List[str]] = ["pdf", "csv", "doc", "docx", "xls", "xlsx", "html", "txt", "md"] 
