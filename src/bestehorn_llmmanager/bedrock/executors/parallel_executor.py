...
from ..models.parallel_structures import (
    BedrockConverseRequest, RegionAssignment, ParallelExecutionContext,
    ParallelProcessingConfig
)
from ..models.bedrock_response import BedrockResponse
from ..exceptions.parallel_exceptions import RequestTimeoutError, ParallelExecutionError
...
class ParallelExecutor:
    ...
    async def execute_requests_parallel(self,
                                      assignments: List[RegionAssignment],
                                      request_map: Dict[str, BedrockConverseRequest],
                                      execute_single_request_func) -> Dict[str, BedrockResponse]:
        """
        ...
        try:
            ...
            responses = self._process_execution_results(
                results=results,
                assignments=assignments
            )
    ...
    def _create_execution_context(self, assignments: List[RegionAssignment]) -> ParallelExecutionContext:
        """
    ...
    async def _execute_single_request_with_context(self,
                                                 request: BedrockConverseRequest,
                                                 assignment: RegionAssignment,
                                                 execute_single_request_func,
                                                 semaphore: asyncio.Semaphore) -> Tuple[str, BedrockResponse]:
        """
        ...
        async with semaphore:
            ...
            try:
                ...
                elapsed_time = self._execution_context.get_elapsed_time_ms() / 1000.0 if self._execution_context else None
                ...
                self._logger.warning(f"Request {request_id} timed out after {self._config.request_timeout_seconds}s")
                
                # Create failed response
                failed_response = BedrockResponse(
                    success=False,
                    warnings=[f"Request timed out after {self._config.request_timeout_seconds} seconds"]
                )
    ...
    async def _execute_request_async(self,
                                   request: BedrockConverseRequest,
                                   assignment: RegionAssignment,
                                   execute_single_request_func) -> BedrockResponse:
        """
    ...
    async def _execute_tasks_with_monitoring(self, tasks: List[asyncio.Task]) -> List[Tuple[str, BedrockResponse]]:
        """
        ...
        for i, result in enumerate(completed_tasks):
            if isinstance(result, Exception):
                ...
                request_id = task_name.replace("request_", "") if task_name.startswith("request_") else f"task_{i}"
                ...
                failed_response = BedrockResponse(
                    success=False,
                    warnings=[f"Task execution failed: {str(result)}"]
                )
    ...
    def _process_execution_results(self,
                                 results: List[Tuple[str, BedrockResponse]],
                                 assignments: List[RegionAssignment]) -> Dict[str, BedrockResponse]:
        """
        ...
        if missing_request_ids:
            ...
            for missing_id in missing_request_ids:
                responses[missing_id] = BedrockResponse(
                    success=False,
                    warnings=["Request execution did not complete"]
                )
    ...
    def _log_execution_completion(self, responses: Dict[str, BedrockResponse]) -> None:
        ...
        if self._execution_context:
            ...
            self._logger.info(f"Parallel execution completed: {successful_count}/{total_count} successful")
