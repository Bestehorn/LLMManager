...
from ..models.aws_regions import AWSRegions, normalize_region_name, get_all_regions
...
class IntegrationTestConfig:
    ...
    timeout_seconds: int = field(default_factory=lambda: int(os.getenv("AWS_INTEGRATION_TIMEOUT", "30")))
    max_retries: int = field(default_factory=lambda: int(os.getenv("AWS_INTEGRATION_MAX_RETRIES", "3")))
    cost_limit_usd: float = field(default_factory=lambda: float(os.getenv("AWS_INTEGRATION_COST_LIMIT", "1.0")))
    skip_slow_tests: bool = field(default_factory=lambda: _get_env_bool("AWS_INTEGRATION_SKIP_SLOW", True))
    ...
    def get_primary_test_region(self) -> str:
        ...
        if not self.test_regions:
            raise IntegrationTestError(
                message="No test regions configured",
                details={"test_regions": self.test_regions}
            )
