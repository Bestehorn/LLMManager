...
from ..models.llm_manager_constants import LLMManagerLogMessages, LLMManagerErrorMessages
...
class AuthManager:
    ...
    def _create_profile_session(self, region: Optional[str] = None) -> boto3.Session:
        ...
        try:
            session = boto3.Session(
                profile_name=self._auth_config.profile_name,
                region_name=region
            )
    ...
    def _create_credentials_session(self, region: Optional[str] = None) -> boto3.Session:
        ...
        try:
            ...
            self._logger.info(
                LLMManagerLogMessages.AUTH_CONFIGURED.format(auth_type="credentials")
            )
    ...
    def _create_iam_role_session(self, region: Optional[str] = None) -> boto3.Session:
        ...
        try:
            ...
            self._logger.info(
                LLMManagerLogMessages.AUTH_CONFIGURED.format(auth_type="iam_role")
            )
    ...
    def get_auth_info(self) -> dict:
        ...
        return {
            "auth_type": self._auth_config.auth_type.value,
            "profile_name": self._auth_config.profile_name if self._auth_config.auth_type == AuthenticationType.PROFILE else None,
