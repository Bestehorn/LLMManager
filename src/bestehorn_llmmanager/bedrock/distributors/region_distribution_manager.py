...
from ..exceptions.parallel_exceptions import RegionDistributionError, ParallelConfigurationError
from ..models.parallel_structures import (
    BedrockConverseRequest, RegionAssignment, LoadBalancingStrategy
)
from ..models.parallel_constants import ParallelLogMessages, ParallelErrorMessages
...
class RegionDistributionManager:
    ...
    def __init__(self, load_balancing_strategy: LoadBalancingStrategy = LoadBalancingStrategy.ROUND_ROBIN) -> None:
        """
    ...
    def distribute_requests(self, 
                           ...
                           target_regions_per_request: int) -> List[RegionAssignment]:
        """
    ...
    def _validate_distribution_parameters(self, 
                                        requests: List[BedrockConverseRequest],
                                        available_regions: List[str],
                                        target_regions_per_request: int) -> None:
        """
        ...
        if target_regions_per_request > len(available_regions):
            raise RegionDistributionError(
                message=ParallelErrorMessages.INSUFFICIENT_REGIONS.format(
                    requested=target_regions_per_request,
                    available=len(available_regions)
                ),
    ...
    def _create_region_assignments(self, 
                                 requests: List[BedrockConverseRequest],
                                 available_regions: List[str],
                                 target_regions_per_request: int) -> List[RegionAssignment]:
        """
    ...
    def _assign_regions_for_request(self, 
                                  request: BedrockConverseRequest,
                                  available_regions: List[str],
                                  target_regions_per_request: int) -> List[str]:
        """
        ...
        if self._load_balancing_strategy == LoadBalancingStrategy.ROUND_ROBIN:
            return self._assign_regions_round_robin(
                available_regions=available_regions,
                target_count=target_regions_per_request
            )
    ...
    def _assign_regions_round_robin(self, available_regions: List[str], target_count: int) -> List[str]:
        """
        ...
        self._last_assigned_region_index = (self._last_assigned_region_index + regions_needed) % len(available_regions)
    ...
    def _assign_regions_least_loaded(self, available_regions: List[str], target_count: int) -> List[str]:
        """
        ...
        sorted_regions = sorted(
            available_regions, 
            key=lambda region: self._region_assignment_counter[region]
        )
    ...
    def _log_distribution_stats(self, assignments: List[RegionAssignment], available_regions: List[str]) -> None:
        """
        ...
        unique_regions_used = len(set(
            region for assignment in assignments for region in assignment.assigned_regions
        ))
        max_assignments_per_region = max(self._region_assignment_counter.values()) if self._region_assignment_counter else 0
    ...
    def optimize_region_assignments(self, 
                                  assignments: List[RegionAssignment],
                                  available_regions: List[str]) -> List[RegionAssignment]:
        """
    ...
    def _optimize_single_assignment(self, 
                                   current_regions: List[str],
                                   available_regions: List[str],
                                   current_load: Dict[str, int]) -> List[str]:
        """
        ...
        for region in current_regions:
            if current_load[region] > avg_load * 1.5:  # High load region
                ...
                for low_load_region in sorted_by_load:
                    if low_load_region not in optimized_regions and current_load[low_load_region] < avg_load:
