...
from ..models.cris_structures import CRISModelInfo, CRISInferenceProfile
...
class CRISHTMLParser(BaseCRISParser):
        ...
        for section_element in expandable_sections:
            ...
            try:
                ...
                self._logger.warning(
                    CRISLogMessages.SECTION_SKIPPED.format(section_id=section_id)
                )
    ...
    def _parse_section_to_variant(self, section: Tag) -> Optional[CRISRegionalVariant]:
        ...
        if not header_attr:
            raise ParsingError(
                CRISErrorMessages.MALFORMED_SECTION.format(section_id=section_id)
            )
        ...
        if not region_mappings:
            raise ParsingError(
                CRISErrorMessages.MISSING_REGION_TABLE.format(section_id=section_id)
            )
    ...
    def _create_merged_model_info_objects(
        ...
        for model_name, variants in regional_variants_by_model.items():
            ...
            try:
                ...
                inference_profiles = self._create_inference_profiles_from_variants(variants=variants)
                ...
                self._logger.debug(
                    CRISLogMessages.SECTION_PARSED.format(model_name=model_name)
                )
    ...
    def _create_inference_profiles_from_variants(
        self, 
        variants: List[CRISRegionalVariant]
