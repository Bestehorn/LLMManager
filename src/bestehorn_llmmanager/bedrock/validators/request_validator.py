...
class RequestValidator:
    ...
    def validate_request_ids(self, requests: List[BedrockConverseRequest]) -> None:
        ...
        if not requests:
            raise RequestValidationError(
                message=ParallelErrorMessages.EMPTY_REQUEST_LIST
            )
    ...
    def _group_requests_by_id(self, requests: List[BedrockConverseRequest]) -> Dict[str, List[BedrockConverseRequest]]:
        """
    ...
    def _find_duplicate_ids(self, id_to_requests: Dict[str, List[BedrockConverseRequest]]) -> Dict[str, List[BedrockConverseRequest]]:
        """
        ...
        return {
            req_id: requests 
            for req_id, requests in id_to_requests.items() 
            if len(requests) > 1
        }
        ...
        for req_id, duplicate_requests in duplicates.items():
            self._logger.error(
                ParallelLogMessages.REQUEST_ID_COLLISION_DETECTED.format(
                    request_id=req_id,
                    collision_count=len(duplicate_requests)
                )
            )
    ...
    def _validate_content_blocks(self, content_blocks: List, message_index: int) -> List[str]:
        ...
        for i, block in enumerate(content_blocks):
            ...
            if not any(content_type in block for content_type in content_types):
                errors.append(f"Message {message_index}, block {i} must have at least one content type")
    ...
    def validate_batch_requests(self, requests: List[BedrockConverseRequest]) -> None:
        ...
        if all_validation_errors:
            error_message = f"Request structure validation failed: {len(all_validation_errors)} errors found"
            raise RequestValidationError(
