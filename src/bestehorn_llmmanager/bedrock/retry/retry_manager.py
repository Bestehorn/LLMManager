                ...
                if (
                    ...
                    try:
                        ...
                        self._logger.info(
                            LLMManagerLogMessages.REQUEST_SUCCEEDED.format(
                                model=model,
                                model_id=model_id_to_use if model_id_to_use else model,
                                region=region,
            ...
            try:
                ...
                if validation_success:
                    ...
                    self._logger.info(
                        LLMManagerLogMessages.REQUEST_SUCCEEDED.format(
                            model=model,
                            model_id=current_args.get("model_id", model),
                            region=region,
                            attempts=attempt_num,
                        )
                    )
                ...
                self._logger.warning(
                    LLMManagerLogMessages.REQUEST_FAILED.format(
                        model=model,
                        model_id=(
                            current_args.get("model_id", model)
                            if "current_args" in locals()
                            else model
                        ),
                        region=region,
                        error=str(error),
