...
class CachePointManager:
    ...
    def inject_cache_points(
        self, messages: List[Dict[str, Any]], model: Optional[str] = None, region: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if model and region and self._config.cache_availability_check:
            ...
            if cache_support is False:
                self._logger.debug(f"Caching not supported for {model} in {region}, skipping injection")
                return messages
    ...
    def _inject_conservative(self, content_blocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, block in enumerate(content_blocks):
            ...
            if estimated_tokens >= self._config.cache_point_threshold and optimal_position == -1:
                ...
                elif estimated_tokens / (estimated_tokens + remaining_tokens) > 0.8:
                    optimal_position = i + 1
    ...
    def _inject_aggressive(self, content_blocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, block in enumerate(content_blocks):
            ...
            if (accumulated_tokens >= cache_threshold and 
                i < len(content_blocks) - 1 and  # Not the last block
                self._is_cacheable_block(block)):
                ...
                for j, block in enumerate(content_blocks):
                    if ConverseAPIFields.CACHE_POINT in block:
                        cache_points.append(
                            CachePointInfo(
                                position=j,
                                cache_type=block[ConverseAPIFields.CACHE_POINT].get("type", "default"),
                                estimated_tokens=self._estimate_tokens_before_position(content_blocks, j)
                            )
                        )
    ...
    def validate_cache_configuration(self, request: Dict[str, Any]) -> List[str]:
        ...
        for message in messages:
            if ConverseAPIFields.CONTENT in message:
                ...
                if (cache_points_in_message > 1 and 
                    self._config.strategy == CacheStrategy.CONSERVATIVE):
                    warnings.append(
    ...
    def remove_cache_points(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for message in messages:
            ...
            cleaned_content = [
                block for block in message[ConverseAPIFields.CONTENT]
                if ConverseAPIFields.CACHE_POINT not in block
            ]
            
            cleaned_message = {
                **message,
                ConverseAPIFields.CONTENT: cleaned_content
            }
