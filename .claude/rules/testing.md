...
Ported from `.kiro/steering/testing.md` (Kiro `inclusion: fileMatch`,
`fileMatchPattern: "test/**/*.py"`). NOTE: Claude Code loads a `paths:` rule when it
reads a matching file (reactive), so the always-on essentials are also summarized in
`CLAUDE.md`.
...
5. **Test Markers** (defined in `pyproject.toml`):
   ...
   - plus the `aws_*` cost/region/model markers in `pyproject.toml`

6. **AWS Integration Tests**:
   - May require AWS credentials
   - Use markers to skip in environments without AWS access
   - Consider cost implications (use the `aws_*_cost` markers)
